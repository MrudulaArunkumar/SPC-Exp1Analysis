...
anova_3.1_3A <- ezANOVA(data = Exp1_phase3.1_3A,
                   ...
                   within = .(Validity,StimulusType),
                   detailed = TRUE)
anova_out(anova_3.1_3A)

ggplot(data = Exp1_phase3.1_3A, aes(x = StimulusType, y = RT, fill = Validity))+
  geom_bar(stat = "summary", fun = "mean", position = position_dodge())+
  scale_fill_manual(values = c("midnightblue","skyblue"))+
  coord_cartesian(ylim = c(400,600))+
  labs(title = "Transfer in Phase 3.1 for participants with all types of awareness",
       xlab = "Stimulus Type",
...
anova_3.2_3A <- ezANOVA(data = Exp1_phase3.2_3A,
                   ...
                   within = .(Validity,StimulusType),
                   detailed = TRUE)
anova_out(anova_3.2_3A)

ggplot(data = Exp1_phase3.2_3A, aes(x = StimulusType, y = RT, fill = Validity))+
  geom_bar(stat = "summary", fun = "mean", position = position_dodge())+
  scale_fill_manual(values = c("midnightblue","skyblue"))+
  coord_cartesian(ylim = c(400,600))+
  labs(title = "Transfer in Phase 3.2 for participants with all types of awareness",
       xlab = "Stimulus Type",
       ylab = "Reaction Time")+theme_classic()
```
...
## Reaction Time
anova31RT_b <- lmer(data = Exp1_3.1_block, RT~1+Validity*StimulusType*BlockNumber+ (1|PROLIFIC_PID))
...
## Error Rate
anova31ER_b <- lmer(data = Exp1_3.1_block, ER~1+Validity*StimulusType*BlockNumber+ (1|PROLIFIC_PID))
