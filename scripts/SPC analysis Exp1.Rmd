...
Phase31_s2 <- ((valmain3)+(valmain3er))+
  plot_annotation(tag_levels = "A", title = "Phase 3.1: S2 Contingency")+
  plot_layout(guides = "collect")&theme_classic()
...
anova31RT <- ezANOVA(data = Exp1_phase3.1,
                   ...
                   detailed = TRUE) 
pander((anova31RT), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type")
...
anova31ER <- ezANOVA(data = Exp1_phase3.1,
                   ...
                   within = .(Validity,StimulusType),
                   detailed = TRUE) 
pander((anova31ER), style = "rmarkdown", caption = "ANOVA(ER) for Phase 3.1 transfer")
...
1. For S1 the CL effect in RT is not significant $t = -1.14, p = 0.87$ but in the ER it is significant $t = 1.97, p = 0.026$.
...
anova3.2RT <- ezANOVA(data = Exp1_phase3.2,
                      ...
                      within = .(Validity,StimulusType),
                      detailed = TRUE)
pander((anova3.2RT), style = "rmarkdown", caption = "ANOVA:RT for 3.2 transfer")
...
anova32ER <- ezANOVA(data = Exp1_phase3.2,
                   ...
                   within = .(Validity,StimulusType),
                   detailed = TRUE) 
pander((anova32ER), style = "rmarkdown", caption = "ANOVA:ER for 3.2 transfer")
...
```{r 3.2tts2,message=FALSE,warning=FALSE}
## For S2
pander(t.test(data = Exp1_phase3.2, RT~Validity,subset = (StimulusType == "S2-R Transfer"), paired = TRUE, alternative = "greater"), style = 'rmarkdown', caption = "t test, RT showing differences between valid and invalid trials for S2 in Phase 3.2-excluding outliers")
pander(t.test(data = Exp1_phase3.2, ER~Validity,subset = (StimulusType == "S2-R Transfer"), paired = TRUE, alternative = "greater"), style = 'rmarkdown', caption = "t test, ER showing differences between valid and invalid trials for S2 in Phase 3.2")
...
1. It is possible that the contingency relation between S1-S2 became extinct with time, as participants might have unlearnt. As shown by [(Schmidt,Crump & Besner,2007)](https://linkinghub.elsevier.com/retrieve/pii/S1053810009002025) the contingencies can rapidly go extinct and in this case the relation with S2 could have been overwritten with R
    ...
    * Could try second order conditioning by switching Phase 1 and Phase 2
...
The Awareness analysis resulted in three conditions/factors depending on the performance on particular questions. The variables were called as:
  
    * S-S aware: for questions related to Phase 1 (Q1, Q3, Q2, Q4)
    * S2-R aware: for questions related to Phase 2 (Q5, Q7, Q6, Q8)
    * S1-R aware: for questions related to Phase 3 (Q9, Q11, Q10, Q12)
    
These involve both the y/n questions as well as the typed answers for the stimuli
  
These values were then merged to the aggregate files from Phase 3.1 and Phase 3.2 to check for transfer effects and were split intp three aggregate files depending on the type of awareness


```{r aware, message=FALSE, warning=FALSE}
...
#creating a summary file that has the aggregate Total Acc
AwareSummary <-Exp1Aware %>% 
  group_by(PROLIFIC_PID,AwareType) %>% 
  summarise(TotalAcc = sum(AwareAccuracy, na.rm = TRUE))

#shows the table of number of participants with what score of TotalAcc per condition
AwareSummary %>% 
...
#using the cutoff score of 2 to decide who is aware and unaware
AwareSummary <- AwareSummary %>% 
  mutate(AwarenessLevel = ifelse(TotalAcc >=2, "Aware","Unaware"))

#finding participants who have all types of awareness
ppall3<-AwareSummary %>% 
  subset(AwarenessLevel=="Aware") %>% count(PROLIFIC_PID)
ppall3 <- ppall3 %>% filter(n == 3)
ppall3 <- ppall3$PROLIFIC_PID

kable(ppall3, caption = "List of participants who had all kinds of awareness (N = 15)")
AwareSummary %>% group_by(AwareType,AwarenessLevel) %>% count() %>% kable(caption = "Number of participants with awareness")

#merging with agg 3.1 and agg 3.2
...
#only keeping pps with all awareness
Exp1_phase3.1_3A <- Exp1_phase3.1 %>% filter(PROLIFIC_PID %in% ppall3)
Exp1_phase3.2_3A <- Exp1_phase3.2 %>% filter(PROLIFIC_PID %in% ppall3)
...
No effect of awareness. Similar to previous results that show that contingency learning is present even when subjects do not notice them.
...
anova31RT_A_S2 <- ezANOVA(data = Exp1_phase3.1_S2,
                   ...
                   detailed = TRUE) 
pander((anova31RT_A_S2), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type")
...
anova31ER_A_S2 <- ezANOVA(data = Exp1_phase3.1_S2,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova31ER_A_S2), style = "rmarkdown", caption = "ANOVA (ER): for Phase 3.1 across stimulus type")
...
anova31RT_A_SS <- ezANOVA(data = Exp1_phase3.1_SS,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova31RT_A_SS), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type based on SS AWareness")
...
anova31ER_A_SS <- ezANOVA(data = Exp1_phase3.1_SS,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova31ER_A_SS), style = "rmarkdown", caption = "ANOVA (ER): for Phase 3.1 across stimulus type")
...
anova31RT_A_S1 <- ezANOVA(data = Exp1_phase3.1_S1,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova31RT_A_S1), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type based on S1 AWareness")
...
anova31ER_A_S1 <- ezANOVA(data = Exp1_phase3.1_S1,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova31ER_A_S1), style = "rmarkdown", caption = "ANOVA (ER): for Phase 3.1 across stimulus type")
...
RT3A <- (S2_31 + SS_31 + S1_31)+
  ...
  scale_x_discrete(labels = c("S1-R Transfer" = "S1-R", "S2-R Transfer" = "S2-R"))
RT3A
...
### When analysing only participants with all types of awareness

Only checking for partcipants who had awareness in all categories

```{r}
anova_3.1_3A <- ezANOVA(data = Exp1_phase3.1_3A,
                   dv = RT,
                   wid = PROLIFIC_PID,
                   within = .(Validity,StimulusType),
                   detailed = TRUE)
pander(anova_3.1_3A, style = "rmarkdown", caption = "ANOVA: RT, for participants who are aware of all associations")

rt_3.1_3A <- ggplot(data = Exp1_phase3.1_3A, aes(x = StimulusType, y = RT, fill = Validity))+
  geom_bar(stat = "summary", fun = "mean", position = position_dodge())+
  scale_fill_manual(values = c("midnightblue","skyblue"))+
  coord_cartesian(ylim = c(400,600))+
  labs(title = "Transfer in Phase 3.1 \n for Reaction time",
       xlab = "Stimulus Type",
       ylab = "Reaction Time")+theme_classic()

anova_3.1_3A_er <- ezANOVA(data = Exp1_phase3.1_3A,
                   dv = ER,
                   wid = PROLIFIC_PID,
                   within = .(Validity,StimulusType),
                   detailed = TRUE)
pander(anova_3.1_3A_er, style = "rmarkdown", caption = "ANOVA: ER, for participants who are aware of all associations")

er_3.1_3A <- ggplot(data = Exp1_phase3.1_3A, aes(x = StimulusType, y = ER, fill = Validity))+
  geom_bar(stat = "summary", fun = "mean", position = position_dodge())+
  scale_fill_manual(values = c("midnightblue","skyblue"))+
  coord_cartesian(ylim = c(0,0.15))+
  labs(title = "Transfer in Phase 3.1 \n for Error Rate",
       xlab = "Stimulus Type",
       ylab = "Error Rate")+theme_classic()


Plot3.1_3A <- (rt_3.1_3A + er_3.1_3A)+
  plot_annotation(tag_levels = "A", title = "Participants with all types of awareness: Phase 3.1")+
  plot_layout(guides = "collect")&
  theme_classic()
...
anova32RT_A_S2 <- ezANOVA(data = Exp1_phase3.2_S2,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova32RT_A_S2), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type")
...
There is a significant 3-way interaction but not in the expected direction
...
anova32ER_A_S2 <- ezANOVA(data = Exp1_phase3.2_S2,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova32ER_A_S2), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.2 across stimulus type")
...
anova32RT_A_SS <- ezANOVA(data = Exp1_phase3.2_SS,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova32RT_A_SS), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type")
...
anova32ER_A_SS <- ezANOVA(data = Exp1_phase3.2_SS,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova32ER_A_SS), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type")
...
anova32RT_A_S1 <- ezANOVA(data = Exp1_phase3.2_S1,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova32RT_A_S1), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type")
...
anova32ER_A_S1 <- ezANOVA(data = Exp1_phase3.2_S1,
                   ...
                   between = .(AwarenessLevel),
                   detailed = TRUE) 
pander((anova32ER_A_S1), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type")
...
### Analyzing participants who showed all types of awareness


```{r warning=FALSE, message=FALSE}
anova_3.2_3A <- ezANOVA(data = Exp1_phase3.2_3A,
                   dv = RT,
                   wid = PROLIFIC_PID,
                   within = .(Validity,StimulusType),
                   detailed = TRUE)
pander(anova_3.2_3A, style = "rmarkdown", caption = "ANOVA: RT for participants with all types of awareness")

rt_3.2_3A <- ggplot(data = Exp1_phase3.2_3A, aes(x = StimulusType, y = RT, fill = Validity))+
  geom_bar(stat = "summary", fun = "mean", position = position_dodge())+
  scale_fill_manual(values = c("midnightblue","skyblue"))+
  coord_cartesian(ylim = c(400,600))+
  labs(title = "Transfer in Phase 3.2 \n for Reaction Time",
       xlab = "Stimulus Type",
       ylab = "Reaction Time")+theme_classic()

anova_3.2_3A_er <- ezANOVA(data = Exp1_phase3.2_3A,
                   dv = ER,
                   wid = PROLIFIC_PID,
                   within = .(Validity,StimulusType),
                   detailed = TRUE)
pander(anova_3.2_3A_er, style = "rmarkdown", caption = "ANOVA: ER for participants with all types of awareness")

er_3.2_3A <- ggplot(data = Exp1_phase3.2_3A, aes(x = StimulusType, y = ER, fill = Validity))+
  geom_bar(stat = "summary", fun = "mean", position = position_dodge())+
  scale_fill_manual(values = c("midnightblue","skyblue"))+
  coord_cartesian(ylim = c(0,0.15))+
  labs(title = "Transfer in Phase 3.2 \n for Error Rate",
...
Plot3.2 <- (rt_3.2_3A + er_3.2_3A)+
  plot_annotation(tag_levels = "A", title = "Participants with all types of awareness")+
  plot_layout(guides = "collect")&
  theme_classic()
...
## Reaction Time
anova31RT_b <- lmer(data = Exp1_3.1_block, RT~1+Validity*StimulusType*BlockNumber+ (1+Validity*StimulusType*BlockNumber|PROLIFIC_PID))
tab_model(anova31RT_b)

#pander(summary(anova31RT_b), style = "rmarkdown", capion = "ANOVA(RT) results including BlockNumber")
...
## Error Rate
anova31ER_b <- lmer(data = Exp1_3.1_block, ER~1+Validity*StimulusType*BlockNumber+ (1+Validity*StimulusType*BlockNumber|PROLIFIC_PID))
tab_model(anova31ER_b)
#pander(summary(anova31ER_b), style = "rmarkdown", capion = "ANOVA(ER) results including BlockNumber")
...
plot3.1Block <- (inter3.1b+inter3.1ber)+
...
anova32RT_b <- lmer(data = Exp1_3.2_block, RT~1+Validity*StimulusType*BlockNumber+ (1+Validity*StimulusType*BlockNumber|PROLIFIC_PID))
tab_model(anova32RT_b)
...
Plot3.2Block <- (inter3.2b+inter3.2ber)+
...
## Last Occurence Analysis

```{r}
Exp1data$SRB = NA

Exp1data <- Exp1data %>% 
  mutate(SRB = ifelse((Phase == 3.1 | Phase == 3.2)& Condition == "S1-R Transfer" & lag(Condition,1)=="S2-R Transfer","Probe-Transfer",NA)) %>% relocate(SRB)
Exp1data %>% count(SRB)   

Exp1data <- Exp1data %>% 
  mutate(SRB = ifelse((Phase == 3.1 | Phase == 3.2)& Condition == "S2-R Transfer" & lag(Condition,1)=="S2-R Transfer","Probe-Same",SRB))
Exp1data %>% count(SRB)

Srbagg3.1 <- Exp1data %>% 
  group_by(SRB,PROLIFIC_PID,Validity) %>% 
  filter(Phase == 3.1) %>% 
  summarise(ER= mean(ER,na.rm=TRUE),
            RT = mean(RT_io, na.rm = TRUE)) %>% 
  rename(StimulusType = SRB)
Srbagg3.1 <- Srbagg3.1 %>% filter(Validity == "valid")

srb31anova <- ezANOVA(data = Srbagg3.1,
                   dv = RT,
                   wid = PROLIFIC_PID,
                   within = .(StimulusType),
                   detailed = TRUE) 
pander((srb31anova), style = "rmarkdown", caption = "ANOVA (RT): for Phase 3.1 across stimulus type")


mean3_1 <- summarySEwithin(Exp1_phase3.1, measurevar= "RT", withinvars=c("Validity","StimulusType"), idvar="PROLIFIC_PID", na.rm=FALSE, conf.interval=.95)

srb31.rt <- ggplot(Srbagg3.1, aes(x=StimulusType, y=RT))+
  geom_bar(stat = "summary",fun = "mean",position = position_dodge())+
  #geom_errorbar(aes(ymin = RT - ci, ymax = RT + ci),width = .1, size = 1,position = position_dodge(.9))+
  scale_fill_manual(values = c("midnightblue","skyblue"))+
  coord_cartesian(ylim = c(400,550))+
